...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* 테마 토글 — aetheris-mode ↔ 기본 (㉓ 섹션에 정의) */
  const _tswBtn = document.getElementById('tsw');
  if (_tswBtn) _tswBtn.addEventListener('click', toggleTheme);
...
/* =====================================================
   ㉓ 테마 토글 — 기본(글래스) ↔ Aetheris 모드
   · 기본 로드: 원래 글래스 배경 (body에 클래스 없음)
   · 토글 클릭: body.aetheris-mode 추가 → Aetheris 스타일
   · localStorage로 새로고침 후에도 유지
===================================================== */
function toggleTheme() {
  const isAetheris = document.body.classList.toggle('aetheris-mode');
  localStorage.setItem('theme', isAetheris ? 'aetheris' : 'default');

  const tsk = document.getElementById('tsk');
  if (tsk) tsk.style.transform = isAetheris ? 'translateX(20px)' : 'translateX(0)';

  console.log('[K-Facility] 테마 →', isAetheris ? '☀️ AETHERIS' : '🌙 기본 글래스');
}
